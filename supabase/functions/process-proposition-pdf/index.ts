/**
 * Edge Function: process-proposition-pdf
 * Phase 1.1: Extracts text from proposition (Prop.) PDF files
 * 
 * Uses the shared PDF extractor service to extract text content from
 * proposition PDFs and stores results in raw_content column.
 * 
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[process-proposition-pdf] Starting - document_id: ${document_id}, limit: ${limit}, dry_run: ${dry_run}`);

    // Build query for propositions needing extraction
    let query = supabase
      .from('documents')
    ...
    if (fetchError) {
      console.error('[process-proposition-pdf] Fetch error:', fetchError);
      throw fetchError;
    }
    ...
    if (!documents || documents.length === 0) {
      return new Response(JSON.stringify({
        ...
        errors: [],
        details: [],
        message: 'No proposition PDFs to process'
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }

    console.log(`[process-proposition-pdf] Found ${documents.length} propositions to process`);
    ...
    for (const doc of documents) {
      try {
        console.log(`[process-proposition-pdf] Processing ${doc.doc_number}: ${doc.title?.substring(0, 50)}...`);
        ...
        if (!isRiksdagenPdf && !isRegeringenPdf && !isPdfExtension) {
          console.log(`[process-proposition-pdf] Skipping non-PDF: ${doc.pdf_url}`);
        ...
        if (!extractionResult.success) {
          console.error(`[process-proposition-pdf] Extraction failed for ${doc.doc_number}: ${extractionResult.message}`);
        ...
        if (!sanitizedText || sanitizedText.length < 100) {
          console.warn(`[process-proposition-pdf] Empty or minimal text for ${doc.doc_number}`);
        ...
        console.log(`[process-proposition-pdf] Extracted ${sanitizedText.length} chars for ${doc.doc_number}`);
        ...
        if (!dry_run) {
          ...
          if (updateError) {
            console.error(`[process-proposition-pdf] Update error for ${doc.doc_number}:`, updateError);
            result.errors.push({
              document_id: doc.id,
              doc_number: doc.doc_number,
        ...
        console.error(`[process-proposition-pdf] Error processing ${doc.doc_number}:`, err);
        result.errors.push({
          document_id: doc.id,
          doc_number: doc.doc_number,
    ...
    console.log(`[process-proposition-pdf] Complete - processed: ${result.processed}, extracted: ${result.extracted}, skipped: ${result.skipped}, errors: ${result.errors.length}`);
    ...
    console.error('[process-proposition-pdf] Fatal error:', error);
    return new Response(JSON.stringify({
