...
## PDF Extraction Metadata for AI Agents

### Overview
All documents in the `documents` table contain rich PDF extraction metadata in the `metadata` JSONB column, providing full transparency about PDF discovery and selection for downstream AI agents.

### Metadata Fields

#### Core Status Fields
- `pdf_status`: 'found' | 'missing' | 'multiple_candidates' | 'extraction_failed'
- `pdf_confidence_score`: 0-100 (higher = more confident in selection)
- `pdf_reasoning`: Array of human-readable reasons for selection
- `pdf_candidates`: Array of alternative PDFs found on page (top 5)
- `extraction_log`: Full decision trail for debugging
- `html_snapshot`: Captured HTML when extraction failed (debugging only)
- `last_extraction_attempt`: ISO timestamp of extraction

### Usage Guidelines for AI Agents

#### Head Detective Agent
- If `pdf_status = 'missing'`, create verification task to check page manually
- If `pdf_confidence_score < 60`, flag process for manual review
- If `pdf_status = 'multiple_candidates'` and count > 3, create disambiguation task
- Review `pdf_reasoning` to understand what signals were detected

#### Timeline Agent
- Only process documents with `pdf_status = 'found'`
- Log confidence score in timeline event metadata for traceability
- Skip timeline extraction if confidence < 30 (no PDF task was created)

#### Metadata Agent
- Check `pdf_reasoning` to understand document type (full report vs summary)
- Use `pdf_candidates` to identify if alternative versions exist (English, kortversion)
- Flag if selected PDF has penalties (kortversion, sammanfattning) applied

#### QA Agent (Future)
- Review all documents with `pdf_confidence_score < 50`
- Verify that `pdf_reasoning` contains 'swedish_full_report' signal for SOUs
- Flag if 'kortversion' or 'english_version' penalties were applied to selected PDF
- Check `html_snapshot` for failed extractions to improve parser

### Confidence Score Interpretation
- **80-100**: High confidence - Swedish full report with doc number match in download section
- **60-79**: Medium confidence - Likely correct but may need verification
- **30-59**: Low confidence - Ambiguous, requires review (task still created)
...
SELECT doc_number, title, 
       metadata->>'pdf_confidence_score' as confidence,
       metadata->>'pdf_reasoning' as reasoning
FROM documents
WHERE (metadata->>'pdf_status') = 'found'
  AND CAST(metadata->>'pdf_confidence_score' AS INTEGER) < 60
ORDER BY CAST(metadata->>'pdf_confidence_score' AS INTEGER) ASC;

-- Find documents with missing PDFs
SELECT doc_number, title, 
       metadata->>'extraction_log' as log,
       url
FROM documents
...
SELECT doc_number, title, 
       ...
       metadata->'pdf_candidates' as candidates
FROM documents
WHERE jsonb_array_length(metadata->'pdf_candidates') > 1
ORDER BY candidate_count DESC;

-- Check PDF selection reasoning
SELECT doc_number, title,
       metadata->'pdf_reasoning' as reasoning,
       metadata->'pdf_candidates'->0 as best_candidate
FROM documents
WHERE (metadata->>'pdf_status') = 'found'
...
The extraction system uses a transparent scoring algorithm:

#### Strong Signals (+10 points each)
- Document number in URL/filename (in download or main content section)
- Document number in link text (in primary sections only)
- Link in "Ladda ner" section with heading

#### Moderate Signals (+5 points)
- Link in structured section (.list--icons, .download)
- URL from regeringen CDN (contentassets)
- Explicit PDF indicator in link text

#### Swedish Full Report Rule (+8 points)
- Primary PDF is Swedish full report (not kortversion, sammanfattning, English)

#### Penalties (-5 to -10 points)
- Kortversion or sammanfattning (-5 each)
- English version or summary (-5)
- Fact sheet/faktablad (-7)
- Appendix for non-appendix documents (-3)

#### Disqualifiers (score = -999)
- External domain (not regeringen.se)
- Cover page only indicators
- Document number found in wrong context (body text, not primary sections)

### Integration Principle
**"Be explicit about what we know, what we don't know, and never pretend."**

All AI agents must read and respect the PDF extraction metadata to maintain system honesty and transparency, which is critical for professional users like government analysts and researchers.
